...
int AppManager::strToInt(std::string numero){
    char* aux;
    int num = 0;
    ...
    for(int i=0; i<strlen(numero.c_str()); i++){
        if(aux[i] < 48 || aux[i] > 57) return -1;

        num *= 10;
        num += static_cast<int>(aux[i]) - 48;
    }
...
void AppManager::MenuUsarios(){
    ...
    do{
        ...
        case 15:    ///baja
            ...
            break;
        case 1:     ///enter
            if(SELECT == OPC::OPCION1)BuscarUsuario();
